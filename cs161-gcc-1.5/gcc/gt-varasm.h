...
{
  struct in_named_entry * const x = (struct in_named_entry *)x_p;
  if (gt_pch_note_object (x, x, gt_pch_p_14in_named_entry, gt_ggc_e_14in_named_entry))
    {
      gt_pch_n_S ((*x).name);
    }
}
...
{
  struct htab * const x = (struct htab *)x_p;
  if (gt_pch_note_object (x, x, gt_pch_p_P14in_named_entry4htab, gt_e_P14in_named_entry4htab))
    {
      if ((*x).entries != NULL) {
        size_t i0;
        for (i0 = 0; i0 != (size_t)(((*x)).size); i0++) {
        ...
        gt_pch_note_object ((*x).entries, x, gt_pch_p_P14in_named_entry4htab, gt_types_enum_last);
      }
    }
}
