...
{
  struct varray_head_tag * const x = (struct varray_head_tag *)x_p;
  if (gt_pch_note_object (x, x, gt_pch_p_P15alias_set_entry15varray_head_tag, gt_e_P15alias_set_entry15varray_head_tag))
    {
      gt_pch_n_S ((*x).name);
      switch ((*x).type)
        {
...
gt_pch_na_alias_invariant (ATTRIBUTE_UNUSED void *x_p)
{
  if (alias_invariant != NULL) {
    ...
    gt_pch_note_object (alias_invariant, &alias_invariant, gt_pch_pa_alias_invariant, gt_types_enum_last);
  }
}
